...
import { TableBuilder } from "./ui/table-builder/TableBuilder";
...
export function Toolbar({ markdown, setMarkdown, editorRef, children }) {
  ...
  const insertLink = (linkMarkdown) => {
    setMarkdown(markdown + linkMarkdown);
  };

  const insertImage = (imageMarkdown) => {
    setMarkdown(markdown + "\n\n" + imageMarkdown);
  };
    ...
    >
      ...
      <div className='flex items-center gap-0.5 bg-[#1a1a2e] rounded-2xl p-1 border border-[#2a2a4a]/30'>
        <motion.button
          ...
          whileHover='hover'
          whileTap='tap'
          className='p-1.5 rounded-xl text-[#6272a4] hover:text-[#8be9fd] hover:bg-[#2a2a4a]/50 transition-all duration-200  '
          title='Insert Table '
