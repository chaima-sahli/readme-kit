            ...
            components={{
              code({ node, inline, className, children, ...props }) {
                ...
                return !inline && match ? (
                  ...
                  <code
                    className='bg-[#2a2a4a]/50 px-2 py-0.5 rounded-lg text-[#50fa7b] text-sm'
                    {...props}
                  >
                    {children}
                  </code>
              ...
              h1({ children }) {
                return (
                  <h1 className='text-4xl font-bold mb-6 text-[#bd93f9] border-b border-[#2a2a4a]/50 pb-4'>
                    {children}
                  </h1>
                );
              },
              h2({ children }) {
                return (
                  <h2 className='text-3xl font-bold mb-4 text-[#bd93f9] border-b border-[#2a2a4a]/50 pb-3'>
                    {children}
                  </h2>
                );
              },
              h3({ children }) {
                return (
                  <h3 className='text-2xl font-bold mb-3 text-[#bd93f9]'>
                    {children}
                  </h3>
                );
              },
              p({ children }) {
                // Check if the paragraph contains multiple images (badges)
                const hasMultipleImages =
                  typeof children === "string" &&
                  (children.match(/!\[.*?\]\(.*?\)/g) || []).length > 1;
                ...
                return (
                  <p className='mb-4 text-[#f8f8f2]/90 leading-relaxed'>
                    {children}
                  </p>
                );
              },
              a({ href, children }) {
                return (
                  <a
                    href={href}
                    className='text-[#8be9fd] hover:text-[#bd93f9] transition-colors duration-200 underline-offset-2 hover:underline'
                  >
                    {children}
                  </a>
                );
              ...
              blockquote({ children }) {
                return (
                  <blockquote className='border-l-4 border-[#bd93f9] pl-6 my-6 text-[#f1fa8c] bg-[#2a2a4a]/20 py-3 rounded-r-2xl'>
                    {children}
                  </blockquote>
                );
              },
              ...
              th({ children }) {
                return (
                  <th className='border-b border-[#2a2a4a]/50 bg-[#1a1a2e] px-4 py-3 text-left text-[#bd93f9] font-semibold'>
                    {children}
                  </th>
                );
              },
              td({ children }) {
                return (
                  <td className='border-b border-[#2a2a4a]/30 px-4 py-3 text-[#f8f8f2]/80'>
                    {children}
                  </td>
                );
              },
